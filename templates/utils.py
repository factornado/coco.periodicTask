...
class Config(object):
    ...
    def todo_callback(self):
        logging.debug('todo callback started')
        r = requests.post('http://localhost:{}/todo'.format(self.get_port()))
        assert r.ok
        if r.json()['createdTasks'] == 0:
            logging.debug('todo callback was empty. Sleep for a while.')
            time.sleep(self.conf['todo_sleep_duration'])
        logging.debug('todo callback finished : '+str(r.json()))

    def do_callback(self):
        logging.debug('do callback started')
        r = requests.post('http://localhost:{}/do'.format(self.get_port()))
        assert r.ok
        if r.json()['nb'] == 0:
            logging.debug('do callback was empty. Sleep for a while.')
            time.sleep(self.conf['do_sleep_duration'])
        logging.debug('do callback finished : '+str(r.json()))
